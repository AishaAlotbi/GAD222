...
public class TextArchitect
{
    ...
    private void Prepare_Fade() //fade text effect
    {
        ...
        for(int i = 0; i < textInfo.characterCount; i++) //Iterate through every character, and set their color to hidden or visible based on if they are pretext or not
        {
            TMP_CharacterInfo charInfo = textInfo.characterInfo[i];

            if(!charInfo.isVisible) //skips spaces in text
                continue;
